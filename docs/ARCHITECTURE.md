...
## ⚠️ Баланс продубльовано в `sim/`

Офлайн-симуляція балансу (`sim/`, запуск `npm run sim`) — **самодостатня**
й має **власну копію** формул у `sim/model.mjs`. Вона навмисне не імпортує
`src/game/balance.js` (sim працює у Node без React/Vite). Тому, **змінюючи
числа балансу, онови обидва місця**, інакше симуляція розійдеться з грою.
Детальніше про самі числа — у [`../BALANCE.md`](../BALANCE.md).
